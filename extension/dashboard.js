...
(() => {
  ...
  function organizationError() {
    ...
    const cases=[
      [/人工固定或不可自动调整的分组，超过上限/,'需保留的分组数超过上限','在模型设置中提高“最多分组数”，或手动将这些记录归入其他项目后再整理；固定归属不会自动改动。'],
  ...
  async function saveModel(test=false) {
    ...
    if($('#model-max-groups')?.validity?.badInput||(maxGroupsValue&&(!Number.isInteger(maxGroups)||maxGroups<1||maxGroups>50)))throw Error('最多分组数请填写 1–50 的整数，或留空保持当前上限。');
