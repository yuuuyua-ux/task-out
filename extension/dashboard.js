...
(() => {
  ...
  const emptyState = () => ({projects:[],items:[],connections:[],connectors:[],sourceStyles:{},migration:{pending:false},onboarding:{step:'start',draft:null},syncSettings:{intervalSeconds:60},model:{baseUrl:'',model:'',rules:'',hasKey:false,autoOrganize:true,maxGroups:5},organization:{status:'unconfigured'},bridge:{url:'http://127.0.0.1:4518',paired:false,status:'disconnected'},suggestions:[]});
  ...
  function organizationError() {
    ...
    const cases=[
      [/分组建议缺少可核对的输入依据/,'模型归组依据缺失或无法核对','模型已响应，但归组依据没有通过校验，这一批未应用。重新加载最新版扩展后重试；已有归属和人工修改保留。'],
      [/人工固定或不可自动调整的分组，超过上限/,'需保留的分组数超过上限','在模型设置中提高“期望分组数”，或手动将这些记录归入其他项目后再整理；固定归属不会自动改动。'],
      [/模型提出的分组超过设置上限/,'模型没有遵守分组上限','重试整理；如果需要更多分组，可在模型设置中提高上限。已保存的整理保留。'],
      [/模型未为每条待合并记录选择保留项目/,'部分记录尚未完成合并','重试整理当前内容，完成剩余分组的合并。已保存的整理和人工固定归属保留。'],
      [/当前在用分组超过上限|当前已有.*在用分组，达到上限/,'需要先合并已有分组','重新整理当前内容以合并自动分组，或在模型设置中提高上限。人工固定归属保留。'],
      [/超时/,'模型响应超时','在模型设置中检查“模型最长等待时间”；较慢模型可设为 300 秒，或更换响应更快的模型。超时不代表模型没有计费。'],
      ...
      [/输出(?:仍)?达到长度上限|自动拆小两次/,'模型返回内容不完整','减少本次整理的内容或简化分组偏好，再重试。'],
  ...
  function organizationStatus() {
    ...
    if(!state.model.baseUrl||!state.model.model)return {status:'unconfigured',label:'本地规则优先 · 模型未配置'};
  ...
  async function organizeNow() {
    ...
    if(visibleItems().some(item=>item.projectId&&!item.archived)){return historyPreview();}
  ...
  function profileFields(profile={}) {
    return `<label class="field">分组说明<textarea id="profile-description" maxlength="500" rows="2">${esc(profile.description||'')}</textarea></label><label class="field">关键词与别名<textarea id="profile-keywords" rows="2" placeholder="每行一个明确项目名或别名">${esc(lines(profile.keywords))}</textarea></label><label class="field">网址范围<textarea id="profile-urls" rows="2" placeholder="https://example.com/project">${esc(lines(profile.urls))}</textarea></label><label class="field">排除词<textarea id="profile-excludes" rows="2">${esc(lines(profile.excludes))}</textarea></label><p class="settings-note">关键词和网址会成为本地规则。多个组同时命中时不会直接归组；禁止外发的来源也可在本机匹配。</p>`;
  }
  const profileForm = () => ({description:formValue('profile-description'),keywords:formValue('profile-keywords'),urls:formValue('profile-urls'),excludes:formValue('profile-excludes')});
  function showHabits() {
    const l=state.learning||{profiles:[],rules:[],feedback:[],metrics:{}};
    const outcomes=l.outcomes||[],total=outcomes.length,assigned=outcomes.filter(o=>o.source).length;
    const rate=(n,d)=>d?`${(100*n/d).toFixed(1)}%`:'暂无数据';
    const metrics=`<p>首次处理 ${total} 条 · 规则覆盖率 ${rate(outcomes.filter(o=>o.source==='rule').length,total)} · 自动归组后人工纠正率 ${rate(outcomes.filter(o=>o.source&&o.corrected).length,assigned)} · 平均分类请求 ${total?((l.classificationRequests||0)/total).toFixed(2):'暂无数据'} 次/条</p><p>平均已知分类费用：${total&&Object.keys(l.costs||{}).length?Object.entries(l.costs).map(([currency,cost])=>`${esc(currency)} ${(cost/total).toFixed(6)}`).join(' / '):'暂无已知费用'}${l.unknownCostRequests?`；${Number(l.unknownCostRequests)} 次费用未知`:''}。仅统计本版首次处理的未归类内容与自动分类请求，重试计入；历史手动整理与进展不计入。</p>`;
    openDialog('分组习惯',`<p>人工固定优先，规则在本机匹配；没有合适目标时保留未归类。已有归属的调整先预览。</p><p>规则归组 ${Number(l.metrics?.ruleAssignments)||0} 次 · 模型归组 ${Number(l.metrics?.modelAssignments)||0} 次 · 人工纠正 ${Number(l.metrics?.corrections)||0} 次</p>${metrics}<div class="stack">${l.profiles.filter(p=>!p.deleted).map(p=>`<article class="panel-card"><h3>${esc(p.name)}</h3><p>${esc(p.description||'尚未设置分组说明')}</p><button data-action="habit-profile" data-id="${esc(p.id)}">说明与规则</button></article>`).join('')||'<p>先新建项目或手动整理几条记录，再积累习惯。</p>'}</div><h3>已记住的规则</h3><div class="stack">${l.rules.filter(r=>r.status!=='deleted').map(r=>`<article class="panel-card"><strong>${esc(l.profiles.find(p=>p.id===r.projectId)?.name||'已删除项目')}</strong><p>${esc([...r.keywords,...r.urls].join('、'))}</p><small>${r.origin==='user'?'用户规则':'从人工调整中学习'} · ${esc({active:'使用中',candidate:'候选，尚未自动使用',paused:'已暂停'}[r.status]||r.status)}${r.supportIds?.length?` · ${r.supportIds.length} 条支持记录`:''}</small><div class="settings-actions"><button data-action="habit-rule" data-id="${esc(r.id)}">查看与编辑</button><button data-action="habit-toggle" data-id="${esc(r.id)}">${r.status==='active'?'暂停':'确认启用'}</button><button data-action="habit-rollback" data-id="${esc(r.id)}">预览撤回历史归属</button><button data-action="habit-delete" data-id="${esc(r.id)}">删除</button></div></article>`).join('')||'<p>尚无规则。移动记录时可选择“以后类似内容也放这里”。</p>'}</div>`, '<button data-action="learning-preview">预览当前内容的历史调整</button><button data-action="dialog-close">完成</button>','habits');
  }
  function showHabitProfile(id) {
    const p=state.learning?.profiles.find(p=>p.id===id&&!p.deleted);if(!p)throw Error('分组已不存在。');dialogContext={id};
    const samples=(state.learning.feedback||[]).filter(f=>f.active&&(f.to===id||f.from===id)).slice(-10).reverse();
    openDialog('分组说明与规则',`<h3>${esc(p.name)}</h3><form id="habit-profile-form">${profileFields(p)}</form><details><summary>人工调整依据</summary>${samples.map(f=>`<p>${f.to===id?'正例':'反例'}：${esc(f.title)}${f.seed?'（已有人工归属，不计作新的学习操作）':''}</p>`).join('')||'<p>暂无人工样本。</p>'}</details>`, '<button data-action="habits">返回</button><button class="primary" form="habit-profile-form" type="submit">保存</button>','habits');
  }
  function showHabitRule(id='',recordId='') {
    const item=itemById(recordId),old=state.learning?.rules.find(r=>r.id===id),projectId=old?.projectId||item?.projectId;
    if(!projectId)throw Error('请先把记录移入一个项目。');dialogContext={ruleId:id,projectId};
    const profile=state.learning?.profiles.find(p=>p.id===projectId);
    openDialog('以后类似内容也放这里',`<p>请核对具体条件，再保存为本地规则。当前这条已固定，其他历史记录不会立即移动。</p><form id="habit-rule-form"><label class="field">关键词<textarea id="rule-keywords" rows="2">${esc(lines(old?.keywords||profile?.keywords?.length&&profile.keywords||[profile?.name||'']))}</textarea></label><label class="field">网址范围<textarea id="rule-urls" rows="2">${esc(lines(old?.urls))}</textarea></label><label class="field">排除词<textarea id="rule-excludes" rows="2">${esc(lines(old?.excludes))}</textarea></label></form>`, '<button data-action="dialog-close">取消</button><button class="primary" form="habit-rule-form" type="submit">确认规则</button>','habits');
  }
  function movedToast(id) {
    showToast('已固定本条归属');clearTimeout(toastTimer);
    const toast=$('#toast');toast.innerHTML=`已固定本条归属 <button data-action="habit-from-record" data-id="${esc(id)}">以后类似内容也放这里</button> <button data-action="undo">撤销</button>`;
    toastTimer=setTimeout(()=>toast.hidden=true,12000);
  }
  async function historyPreview() {
    const ids=visibleItems().filter(i=>!i.archived&&!i.parentId&&!unboundBrowser(i)).map(i=>i.id);
    if(organizing)return;organizing=true;render();try{const result=await api('history-preview',{ids});suggestions=result.suggestions||[];showSuggestionList(suggestions,result.excluded||[],ids);}finally{organizing=false;render();}
  }

  function openProject(id='') {
    const project=state.projects.find(p=>p.id===id)||{name:'',color:'#cdbb9b'};dialogContext={id};
    openDialog(id?'编辑项目':'新建项目',`<p>项目用于整理工作，不绑定本地工作目录。</p><form id="project-form"><label class="field">项目名称<input id="project-name" required maxlength="80" value="${esc(project.name)}" autofocus></label><label class="field">卡片颜色<input id="project-color" type="color" value="${validColor(project.color)}"></label>${profileFields(state.learning?.profiles.find(p=>p.id===id)||{})}</form>`,`${id?'<button class="danger" data-action="project-delete">删除空项目</button>':''}<button data-action="dialog-close">取消</button><button class="primary" type="submit" form="project-form">保存项目</button>`,'project');
  }
  ...
  const modelMaxGroups = () => Number.isInteger(state.model.maxGroups)&&state.model.maxGroups>=1&&state.model.maxGroups<=50?state.model.maxGroups:5;
  function showModel() {
    const model=state.model;dialogContext={originalBase:model.baseUrl||'',hadKey:!!model.hasKey,forgetKey:false};
    openDialog('模型与自动整理',`<p>配置模型后，自动整理网页和已授权的新会话；已有会话只更新进展，保持项目与类型。缺少原名的会话按首条与最新消息生成固定名称，不发送完整历史。</p><p>已有配置时，地址、模型名称、Key、分组偏好和期望分组数留空均保持原值。</p><p class="settings-note">测试只用虚构分类样例，不发送真实记录；测试通过不代表所有真实内容都能正确分类。</p><form id="model-form"><label class="field">模型服务地址（兼容 OpenAI 格式）<input id="model-base" type="url"  value="${esc(model.baseUrl)}" placeholder="https://your-model-service.example/v1"><small>支持 HTTPS 或本机 HTTP；已有地址留空保持不变。</small></label><label class="field">模型名称<input id="model-name"  value="${esc(model.model)}" placeholder="填写模型服务提供的模型名称"></label><label class="field">API Key<input id="model-key" type="password" autocomplete="new-password" placeholder="${model.hasKey?'已保存 · 留空保持原 Key':'本机无认证服务可留空'}"><small id="model-key-hint">Key 只保存在扩展本地，不进入导出和本机服务。更换地址时需填写新 Key 或明确清除，留空不会删除原配置。</small></label><label class="field">期望分组数<input id="model-max-groups" type="number" min="1" max="50" step="1" inputmode="numeric" value="${modelMaxGroups()}" placeholder="默认 5"><small>填写 1–50 的整数；留空保持当前目标。这是整理目标，不是硬上限。优先复用已有组，不同项目可新建并超过目标；人工归属保留。分类参考获取范围内的历史项目，首页只显示当前筛选范围内的项目。</small></label><label class="field">模型最长等待时间（秒）<input id="model-timeout" type="number" min="30" max="600" step="1" value="${Number(model.timeoutSeconds)||120}" placeholder="默认 120"><small>每批含纠正重试，默认 120 秒。较慢模型可设为 300 秒；留空保持不变。延长等待不保证成功，已有取消入口仍有效。</small></label><label class="field">分组偏好<textarea id="model-rules" rows="3" placeholder="例如：优先归入已有项目，项目按工作目标区分">${esc(model.rules)}</textarea></label><label class="check-field"><input id="model-auto" type="checkbox" ${model.autoOrganize!==false?'checked':''}><span>自动整理<br><small class="muted">统一开启或关闭本地规则与模型整理；关闭后可手动整理。已有会话仅更新进展，不覆盖人工归属。</small></span></label></form>`, '<button data-action="model-forget" class="danger">清除 Key</button><button data-action="model-test">保存并测试整理</button><button data-action="dialog-close">取消</button><button class="primary" form="model-form" type="submit">保存设置</button>','model');
  }
  ...
  async function saveModel(test=false) {
    ...
    const configured=!!((baseUrl||state.model.baseUrl)&&(model||state.model.model));
    if(!configured&&(test||model||state.model.model))throw Error('首次配置请填写模型服务地址和模型名称；已有配置留空表示不变。');
    const maxGroupsValue=formValue('model-max-groups'),maxGroups=Number(maxGroupsValue);
    if($('#model-max-groups')?.validity?.badInput||(maxGroupsValue&&(!Number.isInteger(maxGroups)||maxGroups<1||maxGroups>50)))throw Error('期望分组数请填写 1–50 的整数，或留空保持当前目标。');
    if(configured)await requestOrigin(baseUrl||state.model.baseUrl);
    const fields={autoOrganize:formChecked('model-auto')};
    if(maxGroupsValue)fields.maxGroups=maxGroups;
    const timeout=formValue('model-timeout');
    if(timeout){const seconds=Number(timeout);if(!Number.isInteger(seconds)||seconds<30||seconds>600)throw Error('模型等待时间需为 30 到 600 秒的整数。');fields.timeoutSeconds=seconds;}
  ...
  function showSuggestionList(list,excluded=[],ids=[]) {
    dialogContext={ids};
    const content=list.map((s,index)=>{const item=itemById(s.recordId)||{};return `<article class="suggestion-card" data-suggestion-index="${index}"><label class="check-field"><input class="suggestion-selected" type="checkbox" checked><span>${esc(item.title||s.recordId)}</span></label><div class="suggestion-reason">${esc(s.reason||'模型建议')}</div>${['local-rule','grouping-review','rule-rollback'].includes(s.kind)?`<p>目标：${esc(s.patch.projectName||state.learning?.profiles?.find(p=>p.id===s.patch.projectId)?.name||state.projects.find(p=>p.id===s.patch.projectId)?.name||'未归类')}</p>`:''}${Object.prototype.hasOwnProperty.call(s.patch||{},'sessionName')?`<label class="field">会话名称<input class="suggestion-session-name" maxlength="80" required value="${esc(s.patch.sessionName)}"><small>应用后固定，不随新消息变化。</small></label>`:''}${['naming','local-rule','grouping-review','rule-rollback'].includes(s.kind)?'':`<label class="field">项目<select class="suggestion-project">${projectsOptions(s.patch?.projectName?'':s.patch?.projectId??item.projectId??'')}<option value="__new__" ${s.patch?.projectName?'selected':''}>＋ 新建项目</option></select><input class="suggestion-project-name" ${s.patch?.projectName?'':'hidden'} placeholder="新项目名称" value="${esc(s.patch?.projectName)}"></label><label class="field">类型<select class="suggestion-tags">${typeOptions(s.patch?.tags||item.tags)}</select></label><label class="field">一句近况<textarea class="suggestion-summary" rows="2">${esc(s.patch?.summary??item.summary??'')}</textarea></label>`}</article>`;}).join('');
  ...
  async function handleAction(action,target) {
    ...
    switch(action) {
      ...
      case 'habits': showHabits();return;
      case 'habit-profile': showHabitProfile(id);return;
      case 'habit-rule': showHabitRule(id);return;
      case 'habit-from-record': showHabitRule('',id);return;
      case 'learning-preview': return historyPreview();
      case 'habit-toggle': {const r=state.learning.rules.find(r=>r.id===id);if(r.status!=='active'){showHabitRule(id);return;}await mutate('learning-rule-save',{rule:{id,status:r.status==='active'?'paused':'active',confirm:r.status!=='active'}},'规则已更新');showHabits();return;}
      case 'habit-rollback': {const result=await api('learning-rollback-preview',{id});suggestions=result.suggestions||[];showSuggestionList(suggestions,[],suggestions.map(s=>s.recordId));return;}
      case 'habit-delete': await mutate('learning-rule-save',{rule:{id,status:'deleted'}},'规则已删除');showHabits();return;
  ...
  document.addEventListener('submit',async event=>{
    ...
    try{
      if(form.id==='habit-profile-form'){await mutate('learning-profile-save',{id:dialogContext.id,profile:profileForm()});showHabits();return;}
      if(form.id==='habit-rule-form'){await mutate('learning-rule-save',{rule:{...(dialogContext.ruleId?{id:dialogContext.ruleId}:{}),projectId:dialogContext.projectId,keywords:formValue('rule-keywords'),urls:formValue('rule-urls'),excludes:formValue('rule-excludes'),confirm:true,status:'active'}});showHabits();return;}
      if(form.id==='project-form'){await mutate('project-save',{...(dialogContext.id?{id:dialogContext.id}:{}),name:formValue('project-name'),color:formValue('project-color'),profile:profileForm()},'项目已保存');dialog.close();}
      else if(form.id==='record-form'){const initial=dialogContext.initial,patch={},tags=selectedType(formValue('record-tags'));if(formValue('record-alias')!==initial.title)patch.alias=formValue('record-alias');if(formValue('record-project')!==initial.projectId)patch.projectId=formValue('record-project');if(JSON.stringify(tags)!==JSON.stringify(initial.tags))patch.tags=tags;if(formValue('record-summary')!==initial.summary)patch.summary=formValue('record-summary');if(formValue('record-source')!==initial.sourceName)patch.sourceName=formValue('record-source');if(Object.keys(patch).length)await mutate('record-edit',{id:dialogContext.id,patch},'调整已保存');else showToast('没有新的调整');dialog.close();if(patch.projectId)movedToast(dialogContext.id);}
  ...
  function previewAction(action,fields) {
    ...
    if(action==='export')return {data:fields.mode==='config'?{format:'task-out-config',version:2,connections:[],model:{baseUrl:'',model:'',rules:'',autoOrganize:true,maxGroups:5}}:{format:'task-out-records',version:2,projects:C.copy(previewState.projects),sourceStyles:C.copy(previewState.sourceStyles||{}),records:C.copy(previewState.records)}};
