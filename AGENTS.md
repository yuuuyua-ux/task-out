...
17. AI 整理最多使用的分组数可配置为 1–50，默认 5；留空保存不改变设置。上限统计当前网页与自动获取范围内的未归档主会话，不计历史归档/超窗项目定义。超限不强行合并；只有明确规则或带可核对依据的模型建议才可调整，历史归属先预览，只改变项目；人工固定归属与不可自动调整的在用组优先保留。保留组已超过上限时明确提示提高上限或手动调整，不为了凑数改写人工字段。模型输出和应用时均校验总量，分批不能绕过上限。
