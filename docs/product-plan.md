...
| 自动整理 | 默认开启；模型配置完整后，整理已打开网页及近 3 天有活动的已授权主会话。新会话归类一次，已归类会话仅更新近况；可关闭并跨重启保留 |
...
1. 初始化、网页标题或地址变化、来源同步、模型保存后触发防抖检查。默认处理已打开网页和近 3 天的主会话，并再次核对来源获取范围；归档、子会话、待关联网页及未授权来源不进入自动请求。会话按新内容与已归类内容分开处理。
