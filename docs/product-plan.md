...
本版新增：**整理偏好记忆，方案已确认，待实现**。对应第 8.4 节；基础功能继续按下文现有实现与验收状态说明，不因本文更新而视为新增功能已开发完成。
...
已确认的运行方式：**模型由扩展独立直连，本机服务仅负责本地会话读取。** 模型 Key 限于扩展 `chrome.storage.local` 的 `TRUSTED_CONTEXTS`，服务不接收 Key。服务通过 macOS 一键脚本或 `npm start` 启动，用户主动停止，不设置开机常驻。

首版连接器为 Codex rollout 与 Claude 格式 JSONL：具备历史与增量读取，实时运行状态保持未知，尚无经验证的原会话直达能力；`sdk-ts` 不固定识别为 Mana。自动测试与普通网页预览不能代替 Chrome 实机及干净安装验收，当前尚未宣布正式发布。后文的扩展目标与验收标准不应一概视为已验收能力。
...
| 整理记忆入口（待实现） | 首页顶部提供“整理记忆”，模型设置中的分组偏好区提供同一入口；查看、添加、提炼和管理偏好 |
...
3. 根据已有项目、类型和分组偏好生成建议。优先建议已有项目；需要新项目时展示名称候选，由用户选定后创建。第 8.4 节实现后，再加入已确认且适用范围匹配的偏好记忆。
...
| 类型与命名 | “统一使用‘需求方案’，不要同时出现‘PRD’和‘需求设计’” |
...
首页增加“整理记忆”入口，打开后提供“已生效”“待确认”“已停用”视图，以及“新增偏好”“从整理记录提炼”“粘贴资料提炼”。每条记忆可以展开查看依据，明确区分用户明确设置、系统待确认推测和用户已确认的提炼结果。
...
待归类条目可使用候选项目的归类规则，但必须携带明确项目范围，不能把某个项目的局部习惯当作全局规则。分组粒度、类型名称及项目边界均由用户内容决定，不预置固定应用品牌或工作目录规则。
...
本机连接服务仅向已配对且 Origin 匹配的 Task Out 扩展提供受限数据访问；普通网页预览没有该权限。任意网页不能借它读取本机文件；配置指定的范围之外不继续采集，解析后的真实路径也须满足范围限制。
...
| 事项 | 当前建议 | 状态 |
|---|---|---|
| 首批操作系统和浏览器 | macOS + Chrome，Node.js 24.12.x | 需完成真实扩展与干净环境验收 |
| 首批会话连接器 | Codex rollout、Claude 格式 JSONL | 已实现历史、身份与增量读取；虚构样例自动验证已覆盖 |
| 实时状态与直达入口 | 首版保持未知状态，不提供未验证直达 | 后续逐项验证 |
| 本机服务的安装和启动 | macOS 一键脚本、临时配对码、重复启动复用，不设置常驻 | 已实现；干净安装需实机验收 |
