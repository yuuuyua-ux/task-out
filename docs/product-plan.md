...
最多分组数默认 5，可配置 1–50 的整数；未填写保留原值。上限覆盖当前网页与自动获取窗口内的未归档主会话，未归类不占分组名额，归档与超窗项目定义不占名额。已有分组超过上限时，优先保留人工固定或不能自动移动的记录所在组，其余在用组按使用数量与稳定顺序选择保留。模型将多余自动归属合并到保留组，只修改项目，不改类型、名称或近况。受保护分组本身超过上限时提示提高上限或手动调整，不强行合并。新增归类优先复用在用组，剩余名额内才可建组；跨批次、拆分请求与最终应用都检查上限，人工中途修改后重新核对。自动整理在上述活动范围内合并，手动重新整理尊重所选记录范围；筛选外仍有超限分组时说明未完成原因，不擅自移动筛选外记录。自动整理关闭时保存上限不触发模型请求，撤销合并后不立刻自动重做。
